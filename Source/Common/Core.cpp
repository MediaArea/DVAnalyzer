...
string& Core::FileNames ()
{
    ...
    {
        Text+=Ztring(MI->Get(Pos, Stream_General, 0, __T("CompleteName"))).To_UTF8();
    ...
    for (size_t Pos=0; Pos<Count; Pos++)
    {
        ...
        if (MI->Get(Pos, Stream_General, 0, __T("Format"))!=__T("DV")
         ...
         && MI->Get(Pos, Stream_Audio, 4, __T("MuxingMode"))!=__T("DV"))
            ...
            Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("Errors_Stats_End_05"))).To_UTF8();
        else if (Verbosity>=(float)0.3)
            Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("Errors_Stats_End_03"))).To_UTF8();
...
{
    float32 FrameRate=Ztring(MI->Get(File_Pos, Stream_Video, 0, __T("FrameRate_Original"))).To_float32();
    if (!FrameRate)
        FrameRate=Ztring(MI->Get(File_Pos, Stream_Video, 0, __T("FrameRate"))).To_float32();
    ...
    for (size_t Frame=0; Frame<Analysis[File_Pos]->size(); Frame++)
    {
        if (float32_int32s(Verbosity*10)>=Analysis[File_Pos]->at(Frame).Verbosity)
        {
            ...
            if (TimeCode!=0x7FFFF)
            {
                ...
                Errors_Stats_Line+=(Analysis[File_Pos]->at(Frame).TimeCode&0x00000080)?__T(';'):__T(':');
                Errors_Stats_Line+=__T('0')+(Analysis[File_Pos]->at(Frame).TimeCode&0x0000003F)/10;
                Errors_Stats_Line+=__T('0')+(Analysis[File_Pos]->at(Frame).TimeCode&0x0000003F)%10;
            }
            else
            {
...
string& Core::ByFrame ()
{
    ...
    for (size_t Pos=0; Pos<Count; Pos++)
    {
        if (Errors_Stats_WithHeader)
        {
            ...
            Text+="\n\n";
            Common_Header(Pos, Count);
            Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("Errors_Stats_Begin"))).To_Local(); Text+='\n';
        }
        ...
        if (Errors_Stats_WithFooter)
        {
            ...
            if (Verbosity>=(float)0.5)
                Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("Errors_Stats_End_05"))).To_Local();
            else if (Verbosity>=(float)0.3)
                Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("Errors_Stats_End_03"))).To_Local();
        }
...
string &Core::XML()
{
    ...
    Text+="<dvanalyzer>\n";
    Text+="\t<configuration>\n";
    Text+="\t\t<company>"+Ztring().From_UTF8(NameVersion_Text()).SubString(__T(" by "), __T("")).To_Local()+"</company>\n";
    Text+="\t\t<version>"+Ztring().From_UTF8(NameVersion_Text()).SubString(__T("v."), __T(" by ")).To_Local()+"</version>\n";
    ...
    for (size_t File_Pos=0; File_Pos<Count; File_Pos++)
    {
        //XML Header
        Text+="\t<file>\n";
        Text+="\t\t<filepath>"+Ztring(MI->Get(File_Pos, Stream_General, 0, __T("CompleteName"))).To_UTF8()+"</filepath>\n";
        ...
        if (!List.empty())
        {
            ...
            {
                Text+="\t\t<frames_count>"+Ztring(MI->Get(File_Pos, Stream_Video, 0, __T("FrameCount_Speed"))).To_Local()+"</frames_count>\n";
                ...
                {
                    Text+="\t\t<warnings>\n";
                    Text+="\t\t\t<warning code=\"1\">Warning, frame count is maybe incoherant (reported by MediaInfo: "+Ztring(MI->Get(File_Pos, Stream_Video, 0, __T("FrameCount"))).To_Local()+")</warning>\n";
                    Text+="\t\t</warnings>\n";
                }
            }
        }
        ...
        for (size_t Pos=0; Pos<List.size(); Pos++)
        {
            ...
            if (List(Pos, 0).find(__T("Frame count with video error concealment: "))==0)
            {
                ...
                Text+="\t\t\t\t<count>"+List(Pos+1, 0).SubString(__T(": "), __T(" errors")).TrimLeft().To_Local()+"</count>\n";
                Text+="\t\t\t\t<frames_count>"+List(Pos, 0).SubString(__T(": "), __T(" frames")).To_Local()+"</frames_count>\n";
                Pos++;
                Text+="\t\t\t</event>\n";
            }
            ...
            if (List(Pos, 0).find(__T("Frame count with CH1 audio error code: "))==0)
            {
                ...
                Text+="\t\t\t\t<count>"+List(Pos+1, 0).SubString(__T(": "), __T(" errors")).TrimLeft().To_Local()+"</count>\n";
                Text+="\t\t\t\t<frames_count>"+List(Pos, 0).SubString(__T(": "), __T(" frames")).To_Local()+"</frames_count>\n";
                Text+="\t\t\t\t<summary>"+List(Pos+1, 0).SubString(__T("("), __T(")")).To_Local()+"</summary>\n";
                Pos++;
                Text+="\t\t\t</event>\n";
            }
            ...
            if (List(Pos, 0).find(__T("Abs. Time"))==0)
            {
                ...
                {
                    Text+="\t\t\t<edit>\n";
                    Text+="\t\t\t\t<frame_start>"+List(Pos+PosLine, 3).SubString(__T(""), __T(" -")).TrimLeft().To_Local()+"</frame_start>\n";
                    Text+="\t\t\t\t<frame_end>"+List(Pos+PosLine, 3).SubString(__T("- "), __T("")).TrimLeft().To_Local()+"</frame_end>\n";
                    Text+="\t\t\t\t<abstime_start>"+List(Pos+PosLine, 0).To_Local()+"</abstime_start>\n";
                    //Text+="\t\t\t\t<abstime_end>"+List(Pos+PosLine, 0).To_Local()+"</abstime_end>\n";
                    Text+="\t\t\t\t<timecode_start>"+List(Pos+PosLine, 1).SubString(__T(""), __T(" -")).To_Local()+"</timecode_start>\n";
                    Text+="\t\t\t\t<timecode_end>"+List(Pos+PosLine, 1).SubString(__T("- "), __T("")).To_Local()+"</timecode_end>\n";
                    Text+="\t\t\t\t<recdatetime_start>"+List(Pos+PosLine, 2).SubString(__T(""), __T(" -")).TrimRight().To_Local()+"</recdatetime_start>\n";
                    Text+="\t\t\t\t<recdatetime_end>"+List(Pos+PosLine, 2).SubString(__T("- "), __T("")).TrimRight().To_Local()+"</recdatetime_end>\n";
                    Text+="\t\t\t</edit>\n";
...
string &Core::AppleXMLIF(size_t Version)
{
    ...
    for (size_t File_Pos=0; File_Pos<Count; File_Pos++)
    {
        ...
        //XML Header
        Text+="\t<clip id=\""+Ztring(MI->Get(File_Pos, Stream_General, 0, __T("CompleteName"))).To_UTF8()+"\">\n";
        Text+="\t\t<name>"+Ztring(MI->Get(File_Pos, Stream_General, 0, __T("FileName"))).To_UTF8()+"</name>\n";
        Text+="\t\t<duration>"+Ztring(MI->Get(File_Pos, Stream_Video, 0, __T("FrameCount"))).To_Local()+"</duration>\n";
        Text+="\t\t<rate>\n";
        Text+="\t\t\t<ntsc>"; Text+=MI->Get(File_Pos, Stream_Video, 0, __T("Standard"))==__T("NTSC")?"TRUE":"FALSE"; Text+="</ntsc>\n";
        Text+="\t\t\t<timebase>"+Ztring::ToZtring(Ztring(MI->Get(File_Pos, Stream_Video, 0, MI->Get(File_Pos, Stream_Video, 0, __T("FrameRate_Original")).empty()?__T("FrameRate"):__T("FrameRate_Original"))).To_float32(), 0).To_Local()+"</timebase>\n";
        Text+="\t\t</rate>\n";
        Text+="\t\t<file id=\""+Ztring(MI->Get(File_Pos, Stream_General, 0, __T("CompleteName"))).To_Local()+" "+Ztring::ToZtring(File_Pos).To_Local()+"\">\n";
        Text+="\t\t\t<name>"+Ztring(MI->Get(File_Pos, Stream_General, 0, __T("FileName"))).To_UTF8()+"</name>\n";
        Text+="\t\t\t<pathurl>file://localhost";
        #ifdef __WINDOWS__
            Text+="/";
        #endif //__WINDOWS
            Text+=Format::Http::URL_Encoded_Encode(Ztring(MI->Get(File_Pos, Stream_General, 0, __T("CompleteName"))).To_UTF8())+"</pathurl>\n";
        Text+="\t\t</file>\n";
...
{
    Text+=Ztring(MI->Get(Pos, Stream_General, 0, __T("CompleteName"))).To_Local()+'\n';
    Text+='\n';
}
...
void Core::Common_Footer (size_t Pos, size_t Count)
{
    if (MI->Get(Pos, Stream_Video, 0, __T("Format"))==__T("DV") && !MI->Get(Pos, Stream_Video, 0, __T("FrameCount")).empty() && MI->Get(Pos, Stream_Video, 0, __T("FrameCount"))!=MI->Get(Pos, Stream_Video, 0, __T("FrameCount_Speed")))
    {
        ...
        Text+=Ztring(MI->Get(Pos, Stream_Video, 0, __T("FrameCount"))).To_Local();
        Text+=")\n";
    }
    if (MI->Get(Pos, Stream_General, 0, __T("Format"))!=__T("DV"))
    {
        ...
        {
            //Text+='\n');
            //Text+="Warning, the DV track audio configuration doesn't match the container (")+Ztring::ToZtring(DV_Count)+__T(" DV audio tracks, ")+Ztring::ToZtring(Container_Count)+__T(" container audio tracks)\n");
        }
    }
    if (Pos+1<Count)
    {
